...
class XSSDetector:
    def __init__(self, base_url):
        ...
        self.xss_armory = XSSArmory()
        self.screenshot_dir = "screenshots_xss"
    ...
    def crawl(self):
        ...
        for link in soup.find_all('a', href=True):
            href = link['href']
            if href.startswith('/'):
                endpoints.add(self.base_url + href)
            elif href.startswith(self.base_url):
                endpoints.add(href)
        return list(endpoints)

    def test_endpoint(self, endpoint):
        base_payload = "alert('XSS done');"
        payloads = self.xss_armory.generate_payload(base_payload)
        for payload in payloads:
            self.driver.get(endpoint)
            try:
                self.driver.execute_script(payload)
                time.sleep(2)  # Wait for the payload to execute
                if "XSS done" in self.driver.switch_to.alert.text:
                    self.driver.switch_to.alert.accept()
                    screenshot_path = os.path.join(self.screenshot_dir, f"{endpoint.split('/')[-1]}_xss.png")
                    self.driver.save_screenshot(screenshot_path)
                    print(f"XSS found at {endpoint} with payload: {payload}")
                    break
            except:
                continue
...
if __name__ == "__main__":
    base_url = "http://testphp.vulnweb.com/"  # Replace with the target website
    detector = XSSDetector(base_url)
    endpoints = detector.crawl()
    for endpoint in endpoints:
        detector.test_endpoint(endpoint)
    detector.close()